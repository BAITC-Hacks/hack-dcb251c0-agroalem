...
const { postTurnJson, TurnClientError, requireMatchingSession } =
  await loadTypeScript("../src/shared/turn-client/transport.ts");
const { selectExchange } = await loadTypeScript(
  "../src/features/conversation/traceSelection.ts",
);
...
for (const status of [422, 502, 503, 504, 500]) {
  test(`HTTP ${status} remains a failed turn and does not leak body`, async () => {
    await assert.rejects(
      postTurnJson("/api", payload, {
        fetchImpl: async () =>
          new Response("SECRET-LIKE-TEST-BODY", { status }),
      }),
      (error) =>
...
test("caller cancellation is forwarded", async () => {
  const controller = new AbortController();
  const request = postTurnJson("/api", payload, {
    signal: controller.signal,
...
for (const timeoutMs of [0, -1, NaN, Infinity]) {
  test(`invalid timeout ${timeoutMs} is rejected`, async () => {
    await assert.rejects(
      postTurnJson("/api", payload, { timeoutMs }),
      RangeError,
    );
  });
}
...
test("another session is rejected", () =>
  assert.throws(
    () => requireMatchingSession("a", "b"),
    (error) => error.code === "session_mismatch",
  ));

test("empty conversation has no trace", () =>
  assert.equal(selectExchange([], null), null));
test("pending turn cannot borrow previous successful trace", () => {
  const pending = { id: 2 };
  assert.equal(
    selectExchange([{ id: 1, result: { scenario: "fixture" } }, pending], null),
    pending,
  );
});
...
test("missing selected id does not select another turn", () =>
  assert.equal(selectExchange([{ id: 1 }], 99), null));
