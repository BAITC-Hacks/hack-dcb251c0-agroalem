...
test("transcription uses backend multipart and no provider key", async () => {
  ...
  assert.deepEqual(await new AudioApiClient().transcribe(audio()), {
    text: "Тест",
    latency_ms: 12.3,
  });
});
test("speech returns mp3 blob", async () => {
  ...
  assert.equal(
    (await new AudioApiClient().synthesize(" Тест ")).type,
    "audio/mpeg",
  );
});
for (const value of [
  { text: "", latency_ms: 0 },
  { text: "unit", latency_ms: -1 },
  { text: "unit" },
]) {
  test(`invalid transcription ${JSON.stringify(value)}`, async () => {
    globalThis.fetch = async () => json(value);
    await assert.rejects(
      new AudioApiClient().transcribe(audio()),
...
for (const code of [422, 502, 503, 504]) {
  test(`HTTP ${code} stays an error`, async () => {
    ...
    await assert.rejects(
      new AudioApiClient().transcribe(audio()),
      (e) => e instanceof AudioApiError && e.status === code,
    );
  });
}
test("cancelled before fetch makes no network call", async () => {
  ...
  controller.abort();
  await assert.rejects(
    new AudioApiClient().transcribe(audio(), controller.signal),
    (e) => e.code === "cancelled",
  );
});
test("deadline terminates waiting", async () => {
  ...
  await assert.rejects(
    new AudioApiClient("/api", 5).transcribe(audio()),
    (e) => e.code === "timeout",
  );
});
test("voice session mismatch is rejected", async () => {
  ...
  await assert.rejects(
    new AudioApiClient().submitVoice("unit", audio()),
    (e) => e.code === "invalid_contract",
  );
});
