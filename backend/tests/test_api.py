            ...
            RouterOutput.model_validate(
                {
                    ...
                    "clarification_question": (
                        "Это ДТП происходит прямо сейчас или вы оформляете ущерб по КАСКО?"
                    ),
                }
            ),
            12.5,
...
def test_text_turn_returns_trace_and_real_measured_fields() -> None:
    app.dependency_overrides[get_router] = lambda: FakeRouter()
    client = TestClient(app)

    response = client.post(
        "/v1/turn/text",
        json={"session_id": "test-1", "text": "Я только что попал в аварию"},
    )
    ...
    assert body["trace"]["latency_ms"]["router"] == 12.5
...
def test_medium_confidence_returns_clarification() -> None:
    ...
    response = client.post(
        "/v1/turn/text",
        json={"session_id": "test-2", "text": "У меня авария"},
    )
