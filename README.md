...
| Backend текста | В `origin/tim/backend@95eca20`: FastAPI, LLM-router, история, policy и ответ по официальной базе знаний. Новый answer pipeline проверен в живом синтетическом RU-аудиопрогоне. |
| Backend аудио | Опубликованы transcriptions, speech и audio turn. На машине Данила проверен живой STT/router/answer/TTS-путь с синтетической русской речью; это не приёмка физического микрофона и всех языков. |
| Голосовой frontend | Кнопка микрофона, разрешение, запись/стоп/отмена, transcript и trace своей реплики, MP3-плеер с пометкой ИИ. При отказе озвучки текст остаётся; повторяется только TTS. |
...
**Итог: текстовый и голосовой интерфейс подключены; живой синтетический RU-прогон прошёл. Полная приёмка MVP и публичная песочница ещё не завершены.**
...
В текущем голосовом срезе:

- `pnpm check`: форматирование, lint, TypeScript, unit-тесты, 23 текстовых + 16 аудиотранспортных проверок и production build прошли; старый блокер форматирования исправлен;
- `pnpm test:e2e`: **15 passed, 7 skipped** — тестовые API-ответы, симулированный микрофон Chromium, отказ разрешения, сессия text/voice, TTS-only retry и мобильный trace;
- отдельно `REAL_VOICE=1`: **1 passed** против настоящего backend `95eca20` и OpenAI, без подмены API. Синтетическая RU-речь → MediaRecorder → STT → LLM/ответ → TTS → реально начавшееся воспроизведение MP3. Один серверный total около 11,37 с; это не SLA и не время до первого услышанного звука;
- backend в отдельной чистой папке: **110 tests passed**.

Физический микрофон пользователя, Safari/MP4, полный RU/KK/mixed voice smoke, свежий клон и routing baseline остаются открытыми. Accuracy и выполнение latency SLA не заявляются без измерений.
...
Для голоса нажмите **Начать запись**, разрешите микрофон и после реплики нажмите **Остановить и отправить**. Через 60 секунд запись завершится и отправится автоматически; отмена ничего не отправляет. Ответ воспроизводится вручную в плеере. Нужны localhost или HTTPS и разрешение браузера/ОС. Текстовый ввод остаётся дополнительным каналом.
...
1. Принять голосовой UI на физическом микрофоне пользователя; отдельно проверить Safari/MP4.
2. Расширить живые проверки нового backend: RU/KK/mixed, смена темы, уточнение и handoff. Отказ TTS без потери текста/trace уже покрыт контролируемыми ответами, но не вызван намеренно у настоящего провайдера.
